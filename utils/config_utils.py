def validate_tensor_config(condition_config):
    assert 'tensor_condition_config' in condition_config, \
        "Tensor conditioning desired but tensor condition config missing"
    assert 'num_cond_variables' in condition_config['tensor_condition_config'], \
        "num_cond_variables missing in class condition config"
...
def validate_tensor_conditional_input(cond_input, x, num_cond_variables):
    assert 'tensor' in cond_input, \
        "Model initialized with tensor conditioning but cond_input has no tensor information"
    assert cond_input['tensor'].shape == (x.shape[0], num_cond_variables), \
        "Shape of tensor condition input must match (Batch Size, )"
