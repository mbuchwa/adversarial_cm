...
class ResNet_embed_y2cov(nn.Module):
    def __init__(self, block, num_blocks, nc=NC, dim_embed=DIM_EMBED, label_shape=(1,)):
        super(ResNet_embed_y2cov, self).__init__()
        self.in_planes = 64
        if label_shape is None:
            label_shape = (1,)
        elif not isinstance(label_shape, (list, tuple)):
            label_shape = (label_shape,)
        self.label_shape = tuple(label_shape)
        self.label_dim = int(math.prod(self.label_shape)) if self.label_shape else 1
    ...
    def forward(self, x):
        ...
        out = out.view(out.size(0), *self.label_shape)
        return out, features


def ResNet18_embed_y2cov(dim_embed=DIM_EMBED, nc=NC, label_shape=(1,)):
    return ResNet_embed_y2cov(BasicBlock, [2,2,2,2], dim_embed=dim_embed, nc=nc, label_shape=label_shape)

def ResNet34_embed_y2cov(dim_embed=DIM_EMBED, nc=NC, label_shape=(1,)):
    return ResNet_embed_y2cov(BasicBlock, [3,4,6,3], dim_embed=dim_embed, nc=nc, label_shape=label_shape)

def ResNet50_embed_y2cov(dim_embed=DIM_EMBED, nc=NC, label_shape=(1,)):
    return ResNet_embed_y2cov(Bottleneck, [3,4,6,3], dim_embed=dim_embed, nc=nc, label_shape=label_shape)

#------------------------------------------------------------------------------
# map labels to the embedding space
class model_y2cov(nn.Module):
    def __init__(self, dim_embed=DIM_EMBED, num_groups=8, label_shape=(1,)):
        super(model_y2cov, self).__init__()
        if label_shape is None:
            label_shape = (1,)
        elif not isinstance(label_shape, (list, tuple)):
            label_shape = (label_shape,)
    ...
    def forward(self, y):
        y = y.view(y.shape[0], self.label_dim) + 1e-8
        return self.main(y)



if __name__ == "__main__":
    net = ResNet34_embed_y2cov().cuda()
    x = torch.randn(16,NC,IMG_SIZE,IMG_SIZE).cuda()
    out, features = net(x)
    print(out.size())
    print(features.size())

    net_y2h = model_y2cov().cuda()
