...
from CCDM_unified.utils import default, identity, unnormalize_to_zero_to_one, normalize_to_neg_one_to_one, \
    prob_mask_like
...
class GaussianDiffusion(nn.Module):
    ...
    def model_predictions(self, x, t, labels, cond_scale=6., rescaled_phi=0.7, clip_x_start=False):
        model_output, model_output_null = self.model.module.forward_with_cond_scale(x, t, labels, cond_scale=cond_scale,
    ...
    def p_sample(self, x, t: int, labels_emb, cond_scale=6., rescaled_phi=0.7, clip_denoised=True):
        ...
        model_mean, _, model_log_variance, x_start = self.p_mean_variance(x=x, t=batched_times, labels_emb=labels_emb,
    ...
    def p_sample_loop(self, labels_emb, labels, shape, cond_scale=6., rescaled_phi=0.7):
        ...
        for t in tqdm(reversed(range(0, self.sampling_timesteps)), desc='sampling loop time step',
    ...
    def ddim_sample(self, labels_emb, labels, shape, cond_scale=6., rescaled_phi=0.7, clip_denoised=True):
        ...
        for time, time_next in tqdm(time_pairs, desc='sampling loop time step'):
            ...
            pred_noise, x_start, *_ = self.model_predictions(img, time_cond, labels_emb, cond_scale=cond_scale,
    ...
    def convert_y_to_cov(self, labels):
        ...
        b, c, h, w = labels.shape[0], self.channels, self.image_size, self.image_size
        return torch.exp(-self.fn_y2cov(labels).view(b, c, h, w))

    def p_losses(self, x_start, t, *, labels, labels_emb, noise=None, vicinal_weights=None, current_epoch=0):
        ...
        #     # ---- debug plot: 4x4 grid of x with timestep titles ----
        #     import matplotlib
        ...
        #     x_plot = x.detach().float().cpu()  # [16, C, H, W]
        #     t_plot = t.detach().long().cpu().tolist()  # [16]
        #
        #     # expect a batch of 16 for a 4x4 grid
        #     n = min(16, x_plot.shape[0])
        #     rows, cols = 4, 4
        #
        ...
        #         else:
        #             # fallback: first channel as grayscale
        #             ax.imshow(img[0], cmap="gray", interpolation="nearest")
        #         ax.set_title(f"t={t_plot[i]}", fontsize=10)
        #
        #     plt.tight_layout()
        #     out_path = os.path.join(os.getcwd(), "noisy_batch_grid_t.png")
        ...
        #
        #     # ensure CPU numpy for plotting
        #     x_plot = model_out.detach().float().cpu()  # [16, C, H, W]
        #     t_plot = t.detach().long().cpu().tolist()  # [16]
        #
        #     # expect a batch of 16 for a 4x4 grid
        #     n = min(16, x_plot.shape[0])
        ...
        loss = loss * extract(self.loss_weight, t,
                              loss.shape)  # if pred noise and do not clamp SNR, the weight is actually not used.
