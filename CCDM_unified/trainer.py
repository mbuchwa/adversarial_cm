...
class Trainer(object):
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    ## use the vicinal loss
                        ...
                        if self.threshold_type == "soft":
                            vicinal_weights = torch.exp(-self.kappa*(batch_labels-batch_target_labels)**2).to(device)
                        else:
                            vicinal_weights = torch.ones(self.batch_size, dtype=torch.float).to(device)
    ...
    def sample_given_labels(self, given_labels, fn_y2h, batch_size, denorm=True, to_numpy=False, verbose=False, sampler="ddpm", cond_scale=6.0, sample_timesteps=1000, ddim_eta = 0):
        ...
        tmp = 0
        while tmp < nfake:
            batch_fake_labels = torch.from_numpy(given_labels[tmp:(tmp+batch_size)]).type(torch.float).view(-1).to(device)
            self.ema.ema_model.eval()
            with torch.inference_mode():
                if sampler == "ddpm":
