...
# Initialize wandb
wandb.init(
    project="CCDM-Project",  # Change to your project name
    name=f"run_{args.setting_name}_{int(timeit.default_timer())}",  # Unique run name
...
y_visual = torch.from_numpy(y_visual).type(torch.float).view(-1).to(device)
print(y_visual)
