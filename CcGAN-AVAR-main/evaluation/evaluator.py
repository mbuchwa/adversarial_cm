...
class Evaluator:
    def __init__(self, dataset, trainer, args, device):
        ...
        if self.data_name=="UTKFace":
            ...
            if args.img_size==64:
                ...
                self.fid_net_path = "./evaluation/eval_ckpts/UTKFace/metrics_{}x{}/ckpt_AE_epoch_200_seed_2021_CVMode_False.pth".format(args.img_size, args.img_size)
                self.class_net_path = "./evaluation/eval_ckpts/UTKFace/metrics_{}x{}/ckpt_PreCNNForEvalGANs_ResNet34_class_epoch_200_seed_2021_classify_5_races_CVMode_False.pth".format(args.img_size, args.img_size)
                self.regre_net_path = "./evaluation/eval_ckpts/UTKFace/metrics_{}x{}/ckpt_PreCNNForEvalGANs_ResNet34_regre_epoch_200_seed_2021_CVMode_False.pth".format(args.img_size, args.img_size)
            ...
            self.fid_net_path = "./evaluation/eval_ckpts/SteeringAngle/metrics_{}x{}/ckpt_AE_epoch_200_seed_2020_CVMode_False.pth".format(args.img_size, args.img_size)
            self.class_net_path = "./evaluation/eval_ckpts/SteeringAngle/metrics_{}x{}/ckpt_PreCNNForEvalGANs_ResNet34_class_epoch_20_seed_2020_classify_5_scenes_CVMode_False.pth".format(args.img_size, args.img_size)
            self.regre_net_path = "./evaluation/eval_ckpts/SteeringAngle/metrics_{}x{}/ckpt_PreCNNForEvalGANs_ResNet34_regre_epoch_200_seed_2020_CVMode_False.pth".format(args.img_size, args.img_size)
        ...
        for i in range(self.num_eval_labels):
            ...
            fake_labels_i = curr_label*np.ones(self.nfake_per_label)
            self.fake_labels.append(fake_labels_i)
        ##end for i
        self.fake_labels = np.concatenate(self.fake_labels)
        if args.do_subsampling and args.use_dre_reg:
            self.fake_images, _ = self.trainer.rejection_sampling_given_labels(dataset.fn_normalize_labels(self.fake_labels), self.nburnin_per_label, batch_size=args.samp_batch_size, verbose=True)
        else:
            self.fake_images, _ = self.trainer.sample_given_labels(dataset.fn_normalize_labels(self.fake_labels), batch_size=args.samp_batch_size, denorm=True, to_numpy=True, verbose=True)
        stop = timeit.default_timer()
        assert self.fake_images.max()>1.0
        print("\r {}/{}: Got {} fake images. Time spent {:.2f} sec.".format(i+1, self.num_eval_labels, len(self.fake_images), stop-start))
        print("\r Fake images shape:", self.fake_images.shape)
